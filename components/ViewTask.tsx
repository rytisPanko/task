        ...
        <div className='w-full flex flex-col mt-4'>
          {
            viewTask.task?.subtasks?.map((subtask, index) => (
              ...
              }} key={subtask.title} className="w-full min-h-[40px] flex items-center p-3 rounded bg-lightBg dark:bg-darkBg hover:bg-orange/25 hover:dark:bg-orange/25 cursor-pointer mb-2 last:mb-0">
                <input id={`inputCheckbox-${index}`} checked={subtask.isCompleted} onClick={() => { }} onChange={() => { }} type="checkbox" className="min-w-4 w-4 h-4 text-orange cursor-pointer rounded outline-none focus:shadow-none focus:ring-0 mr-4" />
